...
C.push({
  ...
  title:'Asphalt Driveway Sealer Calculator — Gallons, Coats & 5-Gal Pails',
  desc:'Calculate asphalt driveway sealer gallons and 5-gallon pails by area and coats. Check label coverage, plan thin coats, and estimate material cost.',
...
C.push({
  ...
  title:'Thinset Mortar Calculator — 50 lb Bags Needed by Trowel Size',
  desc:'Calculate 50 lb thinset mortar bags for tile floors, walls, and showers. Choose a trowel size, check coverage, and plan material cost.',
...
set('cost',P>0?money(bags*P):'—');`,
  content:{
    ...
    p:'Measure floors, walls, niches, and shower benches separately, then add their areas together. Round up to a whole bag after dividing the total by the selected coverage. Use the coverage printed on your exact thinset bag as the purchase check: trowel notch, tile back profile, substrate flatness, and mixing can change actual yield. A 10% planning cushion can help on uneven substrates or when back-buttering large-format tile. Do not use a single bag estimate for both floor and wall work: measure and purchase for each trowel size separately, then round each material order up to whole bags. Match the trowel to your tile size — the manufacturer lists the recommended notch — and mix only what you can spread and tile within the product&rsquo;s pot life. Before ordering, confirm tile quantity with the <a href="/tile-calculator">tile calculator</a> and plan joints with the <a href="/grout-calculator">grout calculator</a>.',
    faqs:[
...
C.push({
  ...
  desc:'Calculate landscape fabric and weed barrier rolls for garden beds and gravel paths. Account for overlap, roll coverage, square feet, and an estimated cost.',
...
set('cost',P>0?money(rolls*P):'—');`,
  content:{
    ...
    p:'Overlap seams by at least 6 inches so weeds can&rsquo;t push through gaps. A 4 ft-wide roll needs 6 inches of overlap at each seam, so its effective coverage width is closer to 3.5 ft. A 4 ft × 100 ft roll starts with 400 sq ft of material, but its usable coverage is lower once you overlap seams and trim ends; use the manufacturer&rsquo;s stated coverage per roll in the calculator. Run the roll along the longest straight dimension where practical, so you create fewer cross seams. Divide the bed width by the roll&rsquo;s effective width—not its printed width—to estimate how many parallel strips you need, then round the roll count up after checking the product&rsquo;s length and coverage label. Pin the fabric every 18–24 inches and tuck edges under <a href="/landscape-edging-calculator">edging</a> or stones to hold it down. For a gravel path, install and compact the base first; then cut the fabric to follow the finished footprint. Estimate that base with the <a href="/gravel-calculator">gravel calculator</a>.',
    faqs:[
...
set('cost',P>0?money(total*P):'—');`,
  content:{
    ...
    p:'Hardwood (oak, maple, walnut) is commonly priced by the board foot; softwood framing lumber is often sold by the piece or linear foot. For a mixed cut list, calculate each stock size as its own line and add the board-foot totals before applying the waste allowance. For example, four 1 × 6 × 8 ft boards equal 16 board feet; with 10% waste, plan to buy at least 17.6 board feet, then choose available board lengths that cover the cut list. Add 10–15% when your cut list has defects, matching grain, or short offcuts, and buy longer boards only after checking the usable cuts. The price field is your local per-board-foot estimate; confirm the grade, milling, and available lengths on the lumberyard quote before buying. For decking measured by coverage rather than volume, use the <a href="/deck-board-calculator">deck board calculator</a>.',
    faqs:[
...
set('cost',P>0?money(tiles*P):'—');`,
  content:{
    ...
    p:'Check that the tile edge detail matches your existing grid and that you are ordering the same 2×2 or 2×4 module. Sketch the grid layout before ordering so you can confirm the border tiles will not leave an impractically narrow cut at either wall. Box counts and coverage vary by product, so enter the package quantity in the calculator before ordering. Perimeter tiles usually need cuts, so keep a few matching tiles for future replacements. For a few stained or damaged tiles, count the openings you will replace and buy a matching box rather than using the room-area estimate. This calculator estimates replacement tiles only; a new suspended ceiling also needs grid components sized from the room layout.',
    faqs:[
