...
function render(data){
 ...
 const tasksCard='<div class="card"><h2>Your to-do list</h2><p class="muted">Click the button beside a task, then paste the copied request into this Hermes chat.</p><table><tr><th>Status</th><th>What you need to do</th><th>Why</th><th>Get help</th></tr><tr><td class="ok">Done</td><td>Search Console access</td><td>Search demand and rankings are now tracked.</td><td>—</td></tr><tr><td class="ok">Done</td><td>Add <code>Zone Settings: Edit</code> to the Cloudflare token</td><td>Always Use HTTPS is now enabled.</td><td>—</td></tr><tr><td class="bad">Needed</td><td>Join an affiliate program</td><td>Required before we can use paid retailer links and record affiliate revenue.</td><td>'+taskButton('Complete dashboard task: Join an affiliate program')+'</td></tr><tr><td class="bad">Needed</td><td>Create a GA4 property</td><td>Lets us measure visitors and calculator usage, not just Google impressions.</td><td>'+taskButton('Complete dashboard task: Create a GA4 property')+'</td></tr></table></div>';
 const activities=data.activities||[];
 const workRows=activities.length?activities.map(a=>'<tr><td>'+chicagoTime(a.completed_at)+'</td><td>'+a.action+'</td><td>'+a.detail+'</td></tr>').join(''):'<tr><td colspan="3" class="muted">No completed work has been logged yet.</td></tr>';
...
function chicagoTime(value){const iso=value.includes('T')?value:value.replace(' ','T')+'Z';return new Intl.DateTimeFormat('en-US',{timeZone:'America/Chicago',month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit',hour12:true,timeZoneName:'short'}).format(new Date(iso));}
